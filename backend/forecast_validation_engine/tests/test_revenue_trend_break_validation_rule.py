"""Tests for revenue trend-break validation."""
...
from forecast_validation_engine.rules import RevenueTrendBreakValidationRule  # noqa: E402
...
def _execute(
    *,
    ...
    framework = ForecastValidationFramework(
        registry=ValidationRuleRegistry([RevenueTrendBreakValidationRule()])
    )
    context = ValidationContext(
        validation_id="fv_revenue_trend_break",
...
def _trend_break_evidence(output):
    return next(
        evidence
        for evidence in output.result.evidence
        if evidence.evidence_id == (
            "revenue_trend_break_validation:revenue_trend_break"
        )
    )


def test_revenue_trend_break_normal_continuation_passes() -> None:
    output = _execute(
        revenue_result=_series_result("clean"),
    ...
    evidence = _trend_break_evidence(output)
    assert evidence.calculations["forecast_growth_rate"] == pytest.approx(0.10)
    assert evidence.calculations["median_growth_rate"] == pytest.approx(0.095)
    assert evidence.provenance["trend_break_type"] is None
    assert evidence.citations[0].page_number == 292


def test_revenue_trend_break_moderate_deviation_warns() -> None:
    output = _execute(
        revenue_result=_series_result("clean"),
        forecast_value=_forecast_value(0.20),
    )
    execution = output.execution_results[0]
    ...
    evidence = _trend_break_evidence(output)
    assert evidence.provenance["trend_break_type"] == "positive_trend_break"


def test_revenue_trend_break_severe_positive_break_fails() -> None:
    output = _execute(
        revenue_result=_series_result("clean"),
        forecast_value=_forecast_value(0.50),
    )
    execution = output.execution_results[0]
...
def test_revenue_trend_break_severe_negative_break_fails() -> None:
    output = _execute(
        revenue_result=_series_result("clean"),
        forecast_value=_forecast_value(-0.35),
    )
    execution = output.execution_results[0]
    ...
    evidence = _trend_break_evidence(output)
    assert evidence.provenance["trend_break_type"] == "negative_direction_reversal"
    assert evidence.provenance["direction_reversal"] is True


def test_revenue_trend_break_baseline_not_validatable_is_skipped() -> None:
    output = _execute(
...
def test_revenue_trend_break_insufficient_history_is_skipped() -> None:
    output = _execute(
        revenue_result=_series_result(
            "clean",
            values=((2024, 108.0), (2025, 118.8)),
        ),
        forecast_value=130.68,
    )
    execution = output.execution_results[0]
